...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # register buffer to save memo cost because mask is used multiple times but the values are not changed
    ...
    def forward(self, x):
        ...
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # attention formula
        att = att.masked_fill(self.bias[:, :, :T, :T] == 0, float('-inf'))
        # impose causal mask, use T not block_size cuz T in the input might be shorter than block_size
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = att @ v  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C)  # re-assemble all head outputs side by side
        # transpose with contiguous to make pytorch reading tensor more quick
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.attn(self.ln_1(x))  # x = x (residual part) + ...
...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            # lower temperature means lower randomness, higher repetitions, and vice versa
            ...
            # sample from the distribution, not using argmax because it will be so deterministic and many repetitions
